...
fn compile_recursively<T: AsRef<Path>>(source_dir: T, out_dir: T, compiler: &mut shaderc::Compiler) -> bool {
    ...
    for entry in std::fs::read_dir(source_dir).unwrap() {
        ...
        if path.is_file() {
            ...
            let compilation_result = compiler.compile_into_spirv(&shader_contents, shader_kind, path.to_str().unwrap(), "main", None);
            match compilation_result {
                Ok(v) => {
                    ...
                    shader_binary_file.expect("Cannot create shader file").write_all(v.as_binary_u8());
                },
                Err(v) => {
                    eprintln!("Shader {}", v);
