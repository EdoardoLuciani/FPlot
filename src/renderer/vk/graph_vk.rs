...
impl GraphVk {
    ...
    fn recreate_curve_vertex_buffers(&mut self, size: usize) {
    ...
    pub fn fill_graph_buffer(&mut self, x_start: f32, x_end: f32, fun : fn(f32) -> f32) {
        ...
        let mut x = x_start;
        let data_slice = unsafe { std::slice::from_raw_parts_mut(
            (self.host_curve_buffer.allocation.mapped_ptr().unwrap().as_ptr() as *mut [f32; 2]),
    ...
    fn record_static_command_buffers(&self, cmri: &CommandRecordInfo) {
        ...
        for (i, cmd_buf) in cmri.buffers.iter().enumerate() {
            ...
            unsafe {
                ...
                self.bvk.device.cmd_push_constants(*cmd_buf, self.pipeline_layout,
                ...
                self.bvk.device.cmd_draw(*cmd_buf, self.bvk.swapchain_create_info.unwrap().image_extent.width, 1, 4, 0);
